...
exports.createJob = function(req, res) {
  if (!req.body.type)
    return res.send({ error: 'Must provide job type' });

  var job = new Job(req.body.type, req.body.data || {});
  if (req.body.options && req.body.options.attempts)
    job.attempts(parseInt(req.body.options.attempts));
  if (req.body.options && req.body.options.priority)
    job.priority(req.body.options.priority);

  // todo: this assumes success. need to add a success callback to save()
  // success callback would also let us send the job id back in the response
  job.save();
